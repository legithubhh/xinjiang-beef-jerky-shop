    ...
    try {
        ...
        if (cartItems.length === 0) {
            return res.status(400).json({
                success: false,
                error: { code: 'EMPTY_CART', message: '购物车为空' },
                timestamp: new Date().toISOString()
            });
        }
        ...
        const orderResult = await db.execute(`
            INSERT INTO orders (order_no, user_id, total_amount, shipping_fee, final_amount, 
                               payment_method, shipping_name, shipping_phone, shipping_address, message)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [orderNo, userId, totalAmount, shippingFee, finalAmount, paymentMethod,
        ...
        for (const item of cartItems) {
            await db.execute(`
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            `, [orderResult.lastID, item.product_id, item.name, item.price, item.quantity, item.price * item.quantity]);
        }
